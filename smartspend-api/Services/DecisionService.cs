...
public class DecisionService
{
    ...
    public DecisionResult Evaluate(double balance, DateTime nextPayday, double spendAmount)
    {
        var rawDaysLeft = (nextPayday.Date - DateTime.UtcNow.Date).Days;
        var daysLeft = Math.Max(1, rawDaysLeft);
        var safePerDay = Math.Max(0, balance) / daysLeft;
    ...
    public DecisionResult DailyStatus(double balance, DateTime nextPayday)
        => Evaluate(balance, nextPayday, Math.Max(0, balance) / Math.Max(1, (nextPayday.Date - DateTime.UtcNow.Date).Days) * 0.8);
}
